...
- [X] T024 Run all `quickstart.md` scenarios (1-7) manually against a real ImageKit account as final validation — all 7 scenarios (plus `/api/cover`) validated end-to-end against a real account, files confirmed present via ImageKit's own List Files API, then deleted for cleanup. This run caught and fixed a real bug in `listFilesByPrefix` (`src/lib/cdnCache.ts`): the `name : "prefix*"` wildcard search query never matched anything against a real account (mocked tests couldn't catch this — the mock didn't model ImageKit's actual query parser). Fixed by dropping the `*` entirely and relying on the `:` operator's built-in "contains" match, narrowed by the existing client-side `.startsWith(prefix)` filter. Also observed: ImageKit's file list/search index has a short (~seconds) eventual-consistency lag after an upload — a request immediately following a cache-miss render may still miss once or twice before the new file becomes searchable. This is an inherent characteristic of ImageKit's API, not a bug; it self-resolves and only affects near-simultaneous requests for a brand-new (never-before-cached) config.
